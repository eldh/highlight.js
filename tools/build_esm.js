...
async function installPackageJSON() {
  ...
  json.name = '@highlightjs/esm'
  json.description = json.description.concat(' (pre-compiled ESM assets)')
...
async function buildESM(options) {
  ...
  // all the languages are built for the ESM and placed into `/languages`
  ...
  // it really makes no sense to embed ALL languages with the ESM build, it's
  ...
  log(
    'Embedded Lang       :',
    embedLanguages.map((el) => el.minified.length).reduce((acc, curr) => acc + curr, 0),
    'bytes'
  )
  log(
    'All Lang            :',
    languages.map((el) => el.minified.length).reduce((acc, curr) => acc + curr, 0),
    'bytes'
  )
...
async function installLanguages(languages) {
  ...
  await Promise.all(
    languages.map(async (language) => {
      await buildESMLanguage(language)
...
function installStyles() {
  ...
  glob.sync('*', { cwd: './src/styles' }).forEach((file) => {
    ...
    else install(`./src/styles/${file}`, `styles/${file}`)
  })
}
...
async function buildESMLanguage(language) {
