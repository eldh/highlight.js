...
async function buildBrowser(options) {
  ...
  log(
    'Languages           :',
    languages.map((el) => el.data.length).reduce((acc, curr) => acc + curr, 0),
    'bytes'
  )
  if (options.minify) {
    log(
...
async function installDemo(languages, { minify }) {
  ...
  const styles = css.map((el) => ({ name: _.startCase(path.basename(el, '.css')), path: el }))
...
function installDemoStyles() {
  ...
  glob.sync('*', { cwd: './src/styles' }).forEach((file) => {
    if (file.endsWith('.css')) install_cleancss(`./src/styles/${file}`, `demo/styles/${file}`)
    // images, backgrounds, etc
    else install(`./src/styles/${file}`, `demo/styles/${file}`)
  })
}
...
async function buildBrowserHighlightJS(languages, { minify }) {
  ...
  var git_sha = child_process.execSync('git rev-parse HEAD').toString().trim().slice(0, 8)
  ...
  if (minify) {
    ...
    minifiedSrc = [header, tersed.code, ...languages.map((lang) => lang.minified)].join('\n')
